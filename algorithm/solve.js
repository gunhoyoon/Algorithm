...
//   let answer = Array.from({ length: size }, () => 0); // 캐시의 형태를 가짐
...
//     // answer는 처음 다 0으로 세팅
//     // 그래서 중복되는 값을 x가 만났을 때의 경우를 히트 라고 하는데
//     // 그럴땐 가장 최근에 불린 데이터이기 때문에 이전 인덱스 값을 다음에 복사해두고
//     // [0] 번째 자리에 중복된 인덱스 값을 넣어준다.
//     // 일단 중복이 됐을 경우 i의 값을 Pos에 담아두기 때문에

//     // 중복되지 않고 처음 들어가는 값일 경우
//     // 제일 뒤에서 부터 현재 작업[0]까지 하나씩 땡겨준다.
//     // 그래서 처음 캐시 내용은 1 0 0 0 0 상태가 된다.
//     if (pos === -1) {
...
//     }
//     // x가 돌다가 같은 값을 만난 경우
//     // 이때는 그 자리부터 하나씩 앞으로 돌면서 기존 i에 i-1의 값을 넣어준다.
//     // 그러고 다시 0번째에 현재 작업 x 할당
//     else {
//       for (let i = pos; i >= 1; i--) {
//         answer[i] = answer[i - 1];
//       }
